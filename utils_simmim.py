...
def load_checkpoint(config, model, optimizer, lr_scheduler, scaler, logger, device):
    logger.info(f">>>>>>>>>> Resuming from {config.MODEL.RESUME} ..........")
    if config.MODEL.RESUME.startswith('https'):
        checkpoint = torch.hub.load_state_dict_from_url(
            config.MODEL.RESUME, map_location=device, check_hash=True)
    else:
        checkpoint = torch.load(config.MODEL.RESUME, map_location=device)
...
def load_pretrained(config, model, logger, device):
